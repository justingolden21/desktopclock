...
function setTheme(theme) {
	...
	for (const size of sizes) {
		u(`#${size}-ticks`).first().setAttribute('stroke', theme.ticks[size].stroke);
		const r = 27.5 - theme.ticks[size].width / 2;
		u(`#${size}-ticks`).first().setAttribute('r', r);
		u(`#${size}-ticks`).first().setAttribute('stroke-width', theme.ticks[size].width);
		u(`#${size}-ticks`).first().setAttribute('transform', `rotate(-${theme.ticks[size].height})`);
		const numTicks = size == 'sm' ? 60 : size == 'md' ? 12 : 4;
		const dash = (2 * r * Math.PI) / numTicks - theme.ticks[size].height;
		u(`#${size}-ticks`)
			.first()
			.setAttribute('stroke-dasharray', `${theme.ticks[size].height},${dash}`);
	}

	for (const hand of hands) {
		u(`#${hand}-hand`).first().setAttribute('stroke', theme.hands[hand].stroke);
		u(`#${hand}-hand`).first().setAttribute('stroke-width', theme.hands[hand].strokeWidth);
